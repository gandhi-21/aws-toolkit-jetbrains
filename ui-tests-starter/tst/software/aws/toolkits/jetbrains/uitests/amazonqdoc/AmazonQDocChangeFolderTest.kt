...
class AmazonQDocChangeFolderTest {
    init {
        di = DI {
            extend(di)
    ...
    companion object {
        ...
        @AfterAll
        fun cleanup() {
            File(Paths.get("tstData", "Hello", "src", "README.md").toUri()).takeIf { it.exists() }?.delete()
        }
    }
        ...
        Starter.newContext(CurrentTestMethod.hyphenateWithClass(), testCase).apply {
            ...
            .useDriverAndCloseIde {
                try {
                    ...
                    val contents = executeScript(checkFolderChangedScript)

                    Assertions.assertTrue(contents.contains("src"), "Folder change failed: $contents")

                    executeScript(createSubfolderReadmeScript)

                    Thread.sleep(10000)

                    val readmeFile = File(Paths.get("tstData", "Hello", "src", "README.md").toUri())
