...
class ValidateFeatureAvailabilityTest {
    init {
        di = DI {
            extend(di)
    ...
    @Test
    fun `Validate that the feature can be invoked`() {
    ...
    companion object {
        ...
        fun tearDown() {
            ...
            if (File(path).exists()) {
                val process = ProcessBuilder("rm", path.path).start()
                val exitCode = process.waitFor()
                if (exitCode != 0) {
                    println("Warning: rm readme command failed with exit code $exitCode")
